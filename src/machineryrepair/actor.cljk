(ns machineryrepair.actor
  "MachineryRepairPracticeActor — the ISCO-08 7233 community machineryrepair actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one machineryrepair operation request
  ...
  The unconditional invariant: the MachineryRepairPracticeAdvisor can never
  directly commit a record the MachineryRepairPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [machineryrepair.advisor :as advisor]
            [machineryrepair.governor :as governor]
            [machineryrepair.store :as store]))

(defn build-graph
  "Build a compiled MachineryRepairPracticeActor graph. `store` implements
  `machineryrepair.store/Store`. `advisor` implements
  `machineryrepair.advisor/Advisor` (defaults to `mock-advisor`).
